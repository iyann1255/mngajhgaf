...
from DanteMusic import Telegram, YouTube, app
from DanteMusic.misc import SUDOERS, _boot_
from DanteMusic.plugins.play.playlist import del_plist_msg
from DanteMusic.plugins.sudo.sudoers import sudoers_list
...
from DanteMusic.utils.decorators.language import LanguageStart
from DanteMusic.utils.formatters import get_readable_time
from DanteMusic.utils.functions import MARKDOWN, WELCOMEHELP
from DanteMusic.utils.inline import alive_panel, private_panel, start_pannel
        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"<blockquote><b>🔗[ᴛᴇʟᴇɢʀᴀᴍ ғɪʟᴇs ᴀɴᴅ ᴀᴜᴅɪᴏs]({config.SUPPORT_GROUP})  played {count} ᴛɪᴍᴇs\n\n</b></blockquote>"
                    else:
                        msg += f"<blockquote><b>🔗 [{title}](https://www.youtube.com/watch?v={vidid})  played {count} times\n\n</b></blockquote>"
...
"""
        ...
        if await is_on_off(config.LOG):
            ...
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"<blockquote><b>{message.from_user.mention} memulai bot \n\nUser ID : {sender_id}\nUser name: {sender_name}</b></blockquote>",
            )
