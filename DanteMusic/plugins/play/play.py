...
from DanteMusic import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from DanteMusic.core.call import Dante
from DanteMusic.utils import seconds_to_min, time_to_seconds
from DanteMusic.utils.channelplay import get_channeplayCB
from DanteMusic.utils.database import is_video_allowed
from DanteMusic.utils.decorators.language import languageCB
from DanteMusic.utils.decorators.play import PlayWrapper
from DanteMusic.utils.formatters import formats
from DanteMusic.utils.inline.play import (
    livestream_markup,
    playlist_markup,
    slider_markup,
    track_markup,
)
from DanteMusic.utils.inline.playlist import botplaylist_markup
    ...
    if audio_telegram:
        ...
        if message.reply_to_message.document:
            try:
                ...
                print(f"Error Video Telegram: {str(e)}")
                return await mystic.edit_text(
                    _["play_8"].format(f"{' | '.join(formats)}")
                )
...
async def play_music(client, CallbackQuery, _):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            ...
            print(f"Error CallBack Music Stream: {str(e)}")
            return
    try:
        chat_id, channel = await get_channeplayCB(_, cplay, CallbackQuery)
    except Exception as e:
        print(f"Error get_channel: {str(e)}")
        return
    ...
    try:
        ...
        print(f"Error Delete Message: {str(e)}")
        pass
...
async def anonymous_check(client, CallbackQuery):
    try:
        ...
        print(f"Error AnonymousAdmin: {str(e)}")
        return
...
async def play_playlists_command(client, CallbackQuery, _):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            ...
            print(f"Error Callback Playlist: {str(e)}")
            return
    try:
        chat_id, channel = await get_channeplayCB(_, cplay, CallbackQuery)
    except Exception as e:
        print(f"Error Get Channel: {str(e)}")
        return
    user_name = CallbackQuery.from_user.first_name
    await CallbackQuery.message.delete()
    try:
        ...
        print(f"Error Callback answer: {str(e)}")
        pass
    mystic = await CallbackQuery.message.reply_text(
        _["play_2"].format(channel) if channel else _["play_1"]
    )
...
async def slider_queries(client, CallbackQuery, _):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            ...
            print(f"Error Callback Slider: {str(e)}")
            return
    what = str(what)
    rtype = int(rtype)
    if what == "F":
        ...
        try:
            ...
            print(f"Error Callback answer slider: {str(e)}")
            pass
